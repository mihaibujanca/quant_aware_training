...
def _narr_global_axes(
    points_list: list[np.ndarray],
    size: float = 6.5,
    pad_frac: float = 0.2,
) -> Axes:
    """Single fixed axes covering all manifolds (includes 0 so ReLU cuts are visible)."""
    all_pts = np.vstack(points_list)
    xmin, xmax = float(all_pts[:, 0].min()), float(all_pts[:, 0].max())
    ymin, ymax = float(all_pts[:, 1].min()), float(all_pts[:, 1].max())
...
class LayerProgressionScene(Scene):
    ...
    Single fixed axes, explicit operation sequence:
    manifold -> apply W,b -> apply ReLU (with red cut guides) -> next layer.
    """
    ...
    def construct(self) -> None:
        ...
        # Build explicit step list: input -> affine -> relu -> affine -> relu -> ...
        steps: list[tuple[str, np.ndarray, str]] = [("input manifold a0", circle.copy(), "input")]
        a = circle.copy()
        for idx, (W, b) in enumerate(zip(NARR_WS, NARR_BS), start=1):
            z = a @ W.T + b
        ...
        # Fixed axes for the whole sequence so ReLU clipping guides are stable.
        ax = _narr_global_axes([pts for _, pts, _ in steps], size=6.6).shift(DOWN * 0.08)
        ...
        self.play(Create(ax), Create(curve), FadeIn(caption), run_time=1.1)
        self.wait(0.3)
